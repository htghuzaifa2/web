...
async function verify() {
  ...
  const localUrls = urls.map(u => u.replace('https://htg.com.pk', 'http://localhost:3000'));
  ...
  for (const url of localUrls) {
    try {
      ...
      $('a').each((i, el) => {
        ...
        const isExternal = href.startsWith('http') && !href.includes('localhost') && !href.includes('htg.com.pk');
        const isInternal = href.startsWith('/') || href.includes('localhost') || href.includes('htg.com.pk');
